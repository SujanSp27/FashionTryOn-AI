...
class AFlowNet(nn.Module):
    ...
    def forward(self, x, x_warps, x_conds, warp_feature=True):
        ...
        for i in range(len(x_warps)):
              ...
              last_flow = flow
              x_warp = F.grid_sample(x_warp, flow.permute(0, 2, 3, 1), mode='bilinear', padding_mode='border', align_corners=False)
              concat = torch.cat([x_warp,x_cond],1)
              flow = self.netRefine[i](concat)
              flow = apply_offset(flow)
              flow = F.grid_sample(last_flow, flow, mode='bilinear', padding_mode='border', align_corners=False)

              last_flow = F.interpolate(flow, scale_factor=2, mode='bilinear', align_corners=False)
