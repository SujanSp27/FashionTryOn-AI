...
class AlignedDataset(BaseDataset):
    def initialize(self, opt):
        ...
        pairs_candidates = [
            os.path.join(opt.dataroot, 'test_pairs.txt'),
            './test_pairs.txt',
            os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'test_pairs.txt')
        ]
        self.text = './test_pairs.txt'
        for cand in pairs_candidates:
    ...
    def __getitem__(self, index):        
        ...
        input_dict = { 'image': I_tensor,'clothes': C_tensor, 'edge': E_tensor, 'p_name': os.path.basename(self.im_name[index])}
        return input_dict
